...
class Application(Frame):               					# Main window Tkinter setup
    def __init__(self, master=None):
        ...
        #Left Column Stuff
        self.Frame1 = ttk.Frame(self.FrameLeft)
        self.Frame2 = ttk.LabelFrame(self.FrameLeft, text=" Laps ", )
        
        self.Frame1.pack(side=TOP, fill=X)
        self.Frame2.pack(side=BOTTOM, expand=TRUE, fill=BOTH)
																		# Start Button
        self.b1 = ttk.Button(self.Frame2,  text="NO GPS",   command=Btn_Start , style='startr.TButton')
        self.b1.pack(side=BOTTOM, expand=TRUE, fill=BOTH, ipady=20)
        ...
        self.Frame3 = Frame(self.FrameRight)      	# for menu buttons
        ...
        GPS_Label_Status.pack( in_=self.Frame1, fill=X)#, expand=TRUE, )
        #GPS_Label_Lat.pack(     in_=self.Frame1, fill=X)#, expand=TRUE, )
        #GPS_Label_Lon.pack(    in_=self.Frame1, fill=X)#, expand=TRUE, )
        #GPS_Label_Speed.pack( in_=self.Frame1, fill=X)#, expand=TRUE, )
        Ard_Label_Status.pack( in_=self.Frame1, fill=X)#, expand=TRUE, )
        tps_pb.pack(                 in_=self.Frame1, fill=X)#, expand=TRUE, )
...
def btnExit():
    global Record_Lap, Stop_GPS, Record_Thread, GPS_Thread, Arduino_Thread, Output_File, TPS_Log_File
    ...
    if GPS_Thread.is_alive():            # If it does exist, join the thread to update the stopping vars
...
def onselect(event):                    # This is for clicking the listbox, and selecting certain runs
    global Directory, prevIndexCnt, Cone_File#, Lap_Times    # Holds the directory of the run folder
...
def Timing(SF_Y, SF_X, Run_X, Run_Y, Times):        # This handles doing the timing, decinding which points are at st/fin, etc.
    ...
    app.listbox2.insert(END, str(time3.total_seconds()))                # Prints the # of seconds per lap to the times tab.
...
def GPSHandler_Thread():
    ...
    while Stop_GPS == True:                                      # Need nested while-not loops because of GPS disconnect/reconnect reasons
        try:                                                    # Try to connect to the serial port
            ...
            try:
                ser = serial.Serial( port='/dev/serial0', baudrate = 115200, timeout=.2 )
            except:
                #print("Not on serial0")
                    ...
                    for c in ser.read():
                        ...
                        if chr(c) == '\n' and GPS_Line[0] == '$' and GPS_Line[5] == 'C':    # Find the $GPRMC NMEA data
                            try:
                                ...
                                GPS_Output_Speed_Raw = str('{:.0f}'.format(round(int(GPS_Parsed.spd_over_grnd)*1.151))) # speed for big screen, kts->mph
    ...
    while Stop_GPS == True:
        try:
            ...
            if len(ports) == 0:                             # List is empty? Wait a while
                ...
                for p in ports:
                    try:
                        #print("ardDescription: " + str(p.description))
                        if str(p.device) == GPS_Loc:                         # If it's an arduino port, skip it
                            #print("Already GPS port" + str(p))
                            time.sleep(1)                       
                            continue
                        
                        elif "CH340" or "USB" in p.description: 
                            time.sleep(.1)
                            
                            Arduino_Status.set('Ard Status: trying')
                            Ard_Loc = p.device                      # Lists the device name COM5, etc.
                            #print("Arduino found on: " + str(Ard_Loc))
                            ardSer = serial.Serial(port=str(Ard_Loc), baudrate=115200, timeout=3)
                            p = ""                                  # Clear the Port             
                            ...
                            while ardSer.isOpen() == True and Stop_GPS == True:     # This is the main TPS recording loop
                                ...
                                #print(str(line))
                                Arduino_Status.set("Ard Status: Connected")
                    ...
                    if str(port.device) == Ard_Loc:                         # If it's an arduino port, skip it
                        ...
                        with serial.Serial(port[0], 115200, timeout=1) as ser:
                            ...
                            try:
                                ...
                                pynmea2.parse(line.decode('ascii', errors='replace').strip('\r\n'))
                                GPS_Loc = port.device
                                return port
                            except Exception as e:
                                #print('Error: ' + str(e))
